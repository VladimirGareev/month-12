describe ("string page testing", ()=>{
    ...
    it('удаление элемента работает корректно', ()=>{
        ...
        cy.get('input').type('1');
        cy.get('button').contains('Добавить').click();
        cy.get('[data-testid="small"]').should('have.length', 2).eq(1).should('contain','1').and("have.css","border", "4px solid rgb(0, 50, 255)");
        cy.get('button').contains('Удалить').click();
        cy.get('[data-testid="small"]').eq(1).should("have.css","border", "4px solid rgb(210, 82, 225)");
        cy.get('[data-testid="head"]').should('have.length', 1).and('contain', 'top');
        cy.get('button').contains('Удалить').click();
        cy.get('[data-testid="small"]').should("have.css","border", "4px solid rgb(210, 82, 225)");
