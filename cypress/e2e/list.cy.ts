describe("string page testing", () => {
  beforeEach(() => cy.visit("http://localhost:3000/list"));
      ...
      cy.get('[data-testid="head"]')
        ...
        .within(() => {
          cy.get('[data-testid="small"]')
            .should("contain", "S")
            .should("have.css", "border", "3.84px solid rgb(210, 82, 225)");
        });
    ...
    it("проверка удаления элемента из head", () => {
      cy.get('[data-testid="button"]').eq(2).click();
      cy.get('[data-testid="circle"]')
        ...
        .within(() => {
          ...
          cy.get('[data-testid="tail"]')
            ...
            .should("contain", "A")
            .within(() => {
              cy.get('[data-testid="small"]').and(
                "have.css",
                "border",
                "3.84px solid rgb(210, 82, 225)"
              );
            });
        });
      cy.get('[data-testid="circle"]')
    ...
    it("проверка удаления элемента из tail", () => {
      cy.get('[data-testid="button"]').eq(3).click();
      cy.get('[data-testid="small"]')
        .eq(0)
        .should("have.css", "border", "3.84px solid rgb(210, 82, 225)");
      cy.get('[data-testid="small"]')
      ...
      cy.get('[data-testid="circle"]')
        ...
        cy.get('[data-testid="circle"]').should('have.length',4).eq(3).within(()=>{
  ...
  it("проверка удаления элемента по индексу", () => {
    cy.get("input").eq(1).type("2");
    cy.get('[data-testid="button"]').eq(5).click();
    cy.get('[data-testid="small"]')
      .eq(0)
      .should("have.css", "border", "3.84px solid rgb(210, 82, 225)");
    cy.get('[data-testid="small"]')
